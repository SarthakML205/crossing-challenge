...
Two modes:
  ...
  Local dev grading:
      python grade.py
          Reads data/dev.parquet from the reference dataset, prints the
          composite score.
...
on Eval by construction. Lower is better. The numbers printed here must
match what the real grader prints on the same predictions — this file's
scoring logic is held in lock-step with the grader's score.py.
"""
...
# Local dev: point at the shared reference data folder (sibling to this repo).
# Docker grader mode always passes an explicit <input_parquet> argument, so
# this constant is only used for the no-argument local-dev invocation.
...
# KEEP IN SYNC with the grader's score.py and the reference grade.py.
BCE_FLOOR = 0.2488
ADE_FLOOR = 49.80
